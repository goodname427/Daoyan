...
秘书默认使用零-token 本地路由，根据方向中的文档、界面、核心和架构风险信号建立最小计划。模糊的“继续”会先解析为 `docs/status.md` 中的首个后续任务。只有显式使用 `--deep-plan` 时才调用 Luna low，并只向它提供本地生成的压缩项目摘要。同一层级的线性任务会在本地合并，避免多个 Agent 重复读取相同上下文。执行失败最多自动升级一次，随后保留报告并停止。
