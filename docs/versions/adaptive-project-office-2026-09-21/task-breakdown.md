...
旧状态缺少阶段策略，范围修订 2 继续采用 `legacy-unknown` 的保守完整执行。本清单只拆分审核后仍需实现或演练的工作；不以已存在的纵向切片、策划文档或后续 QA 结论制造占位任务。
...
| `version-qa-reuse`     | 测试 Agent | `feature-quality-loop`                   | `scripts/version-lifecycle.ts`、`scripts/secretary-notice-guard.ts`；对应测试                                                          | `npm run typecheck`；`npm test -- test/version-lifecycle.test.ts test/secretary-notice-guard.test.ts`；`npm run test:e2e`；`npm run build`                                |
