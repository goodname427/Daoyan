...
旧状态缺少阶段策略，范围修订 4 继续采用 `legacy-unknown` 的保守完整执行。已批准的审计澄清继续收束在既有工作项；秘书状态对账缺陷新增为一个明确的实现工作项，不另开版本、Feature 或制作人门禁。本清单只拆分审核后仍需实现或演练的工作；不以已存在的纵向切片、策划文档或后续 QA 结论制造占位任务。
...
| `selective-recovery`               | Feature PM | `evidence-contract`、`task-profile`                        | `scripts/agent-dispatcher.ts`、秘书状态/守卫、生命周期；含真实 dispatcher 恢复及对应状态测试                                           | `npm run typecheck`；`npm test -- test/agent-dispatcher-recovery.test.ts test/secretary-state.test.ts test/secretary-notice-guard.test.ts test/version-lifecycle.test.ts` |
...
| `secretary-backlog-reconciliation` | Feature PM | `evidence-contract`、`selective-recovery`、`public-timing` | `scripts/secretary-state.ts`、`scripts/secretary-notice-guard.ts`（含 `/api/dashboard` 投影）；对应三组测试                            | `npm run typecheck`；`npm test -- test/secretary-state.test.ts test/secretary-notice-guard.test.ts test/secretary-dashboard.test.ts`                                      |
| `feature-quality-loop`             | Feature PM | 前五项                                                     | `scripts/agent-dispatcher.ts`、`scripts/agent-routing.ts`、生命周期/guard；对应三组测试                                                | `npm run typecheck`；`npm test -- test/agent-routing.test.ts test/version-lifecycle.test.ts test/secretary-notice-guard.test.ts`；`npm run verify`；`npm run verify:full` |
...
- `secretary-backlog-reconciliation` 以兼容迁移和原子写入关闭手机 Sites/手机优先候选（移动入口已由钉钉替代）及已交付的桌面工作台增强候选（关联现有交付记录和提交）；后续取消、替代、合并或交付事实按稳定关联关系反向闭合原始候选。统计与问答先做状态覆盖和去重，错误排期须发出可去重更正通知；回归覆盖取消覆盖、交付反向闭合、重启持久化、任务计数与问答结果。
