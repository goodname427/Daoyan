...
| `secretary-backlog-reconciliation` | Feature PM | `evidence-contract`、`selective-recovery`、`public-timing` | `scripts/secretary-state.ts`、`scripts/secretary-notice-guard.ts`、`scripts/secretary-dashboard.ts`；对应三组测试                      | `npm run typecheck`；`npm test -- test/secretary-state.test.ts test/secretary-notice-guard.test.ts test/secretary-dashboard.test.ts`                                      |
